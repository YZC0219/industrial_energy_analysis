...
**它不重跑装载以外的任何东西, 也不会污染工作区**: 装载是 upsert + 幂等的
(见 `import_mysql.py`), 所以重复跑结果一致; 产物会被覆盖成新鲜的一份,
这正是本脚本要验的。

> 注意 `load_warehouse` 走的是增量模式(`--incremental`)。在模拟环境里
> `generate_raw_data` 每次全量产出的 CSV 与上一批逐字节相同, 所以增量批次里
> 没有新行, 装载结果与全量一致 —— 这让本脚本无需改动即可通过。但也意味着
> **它验不到水位线真正推进的那条路径**; 那条路径由
> `tests/test_import_mysql.py` 的水位线测试覆盖。
