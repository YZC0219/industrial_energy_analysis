...
CHECKS = {
 ...
 "dwd_required": "SELECT count(*)=0 FROM energy_dwd.dwd_energy_consumption_merge_stage WHERE batch_id='{d}' AND (record_date IS NULL OR workshop_code IS NULL OR energy_code IS NULL OR coalesce(is_deleted,0) NOT IN (0,1) OR (coalesce(is_deleted,0)=0 AND consumption<0))",
