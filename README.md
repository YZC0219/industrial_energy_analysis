...
阶段二在保留 2σ、产量基线和 CUSUM 的基础上，增加统一特征工程、滚动时序验证、季节性基线、树模型与深度时序模型对照，并建设只依据指标字典、SQL 结果和异常证据回答的可追溯归因助手。当前已落地无泄漏特征管道、滚动验证框架、7 日季节性朴素基线、LightGBM，以及带引用白名单、证据不足降级和审计记录的 RAG 归因入口；预测模型已接入同一 Airflow/CI 证据链。LSTM/Transformer 仍在实现，在线 LLM 效果需配置模型端点后单独评测。详见 [阶段二设计说明](docs/阶段二_预测与智能归因.md)。
...
- [x] 使用 Spark SQL 重写核心清洗和聚合任务；结果一致性与性能实测待 Linux 集群验收；
- [x] 使用 DataX 构建 MySQL 与 Hive 的离线同步链路，记录全量、增量和失败重跑策略；
- [x] 扩展现有 Airflow DAG，加入分层依赖、质量门禁、失败告警和补数流程。

> 已完成实现与离线回归；由于 Spark/Hive 尚未从本仓库在 Linux 环境实际运行，三引擎实测与性能比较仍待集群验收，不将代码存在等同于集群验收完成。
