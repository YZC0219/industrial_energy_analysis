...
| 2 | **数据分析与算法** | Q01~Q23 业务分析、2σ 异常检测、考虑产量的能耗基线、CUSUM 异常检测、算法对比实验 |
...
| 3 | MySQL 星型模型 | ✅ | 3 维表 + 2 事实表 + 3 视图（`sql/create_table.sql`） |
...
| 8 | Q01～Q23 业务分析 | ✅ | `sql/analysis.sql`，23 份基线逐字节锁定 |
| 9 | 现有 2σ 异常检测 | ✅ | Q16（单耗异常日）、Q22（环比突增） |
| 10 | 考虑产量等因素的能耗基线 | ⬜ | 现在只有固定阈值(25%)和 2σ，无「按产量折算的期望能耗」模型 |
| 11 | CUSUM 异常检测 | ⬜ | `analysis.sql` 里搜不到 CUSUM |
| 12 | 算法对比实验 | ⬜ | 2σ vs 产量基线 vs CUSUM 的检出率/误报率/提前量。**纯分析层，并行安全** |
| 13 | 静态 HTML 可视化报告 | ✅ | `output/report.html`，自包含，含悬停/表格视图/主题切换 |
| 14 | 日期和车间动态筛选 | ⬜ | 有工作日/周末/节假日切换，但无日期区间与车间下拉筛选控件 |
| 15 | 产品与班组多维查询 | ⬜ | **依赖 #6**，维度不存在则无从查起 |
| 16 | 异常详情展示 | ⬜ | 有告警区，但无「点进去看某条异常明细」 |
...
**路线 A —— 从 #6「产品、班组维度」开始。**
它是数据模型的缺口，**卡住 #15**（多维查询依赖它），也是 #10（产量基线）
更细粒度的前提。动手前先确认毕设范围是否真的要包含产品/班组；
若可砍掉 #6 和 #15，能省掉大半工作量。

**路线 B —— 从 #12「算法对比实验」开始。**
纯分析层、不碰数据模型、与任何其他项并行安全，且答辩时最容易出彩
（有对比表、有量化结论）。缺点是 #10/#11 得先做出来才有东西可对比。
