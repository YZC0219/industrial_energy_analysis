...
| 8 | Q01～Q24 业务分析 | ✅ | `sql/analysis.sql`，24 份基线逐字节锁定 |
...
**#9 2σ 异常检测的完整可视化**。原来只有 Q16 一张 `LIMIT 50` 的告警清单，
计算做了但报告里看不见。本轮：

- 新增 **Q24**（纯新增，不动现有 23 条）：返回 7 个车间 × 731 天的**完整**单耗
  序列，并带回车间自身的 μ/σ 与超限标记。SQL 里算好阈值，前端不重复实现。
- 报告新增「**异常**」节：单耗曲线 + μ ± 2σ 控制带 + 超限点标红，**可切换车间**，
  悬停显示日期/单耗/μ/σ/Z 值/产量，表格视图只列超限日。顺带覆盖了 #16 的一部分。
- 护栏从 23 放宽到 24（`tests/test_analysis_snapshot.py` 三处），新增 1 份基线。

**过程中发现的两件事（下轮要注意）**：

1. **Q16 的 `LIMIT 50` 在截断结果**。全量超限日是 **251 天**，Q16 只返回前 50。
   Q16 的 50 行是 Q24 超限集的真子集，μ/σ 完全一致 —— 口径没问题，是 `LIMIT` 的问题。
   若告警清单要完整，改 Q16 时记得同步更新它的基线。
2. **小基数车间会放大 Z 值**。μ−2σ 在 4 个车间里是**负数**（如包装 μ=0.0934、
   σ=0.0654），负单耗无意义，所以控制带下界夹到 0。报告里已用 `.note.warn`
   说明"Z 值大 ≠ 问题严重"，答辩时这是个能讲清楚的方法论点。

## 下一步做什么

**有两条路，按你的判断挑一条：**

**路线 A —— 接着做 #10「考虑产量的能耗基线」，再 #11 CUSUM，最后 #12 对比。**
这是同一条线的三个递进项，#12 依赖前两个。纯分析层，不碰数据模型，与任何
其他项并行安全，且答辩时最容易出彩（有对比表、有量化结论）。
本轮已经把 #9 的可视化做完了，正好可以给 #10/#11 复用同一套控制图组件。

**路线 B —— 从 #6「产品、班组维度」开始。**（原来的路线 A）
它是数据模型的缺口，**卡住 #15**（多维查询依赖它），也是 #10（产量基线）
更细粒度的前提。但这条线要动星型模型 + 事实表外键 + 重新生成数据，
**会让 24 份基线全部失效**，代价比路线 A 大得多。动手前先确认毕设范围
是否真的要包含产品/班组；若可砍掉 #6 和 #15，能省掉大半工作量。
...
│   ├─ test_analysis_snapshot.py# Q01~Q24 结果回归基线(需要 MySQL)
│   ├─ update_baseline.py       # 基线更新工具
│   └─ baseline/                # 24 份查询结果快照, 作为"正确答案"
...
| `mysql` | 项目的分析仓库（星型模型 + 24 条查询跑在这里），映射到宿主机 `3307` | `mysql-data` 卷 + `./output`（只读）|
...
报告分 9 段：总览 → 结构 → 趋势 → 成因 → 成效 → 损耗 → 风险 → 异常 → 方法。
...
| `test_analysis_snapshot.py` | Q01~Q24 结果与基线逐行比对 | 是 |
...
是**回归基线**：前者守清洗产物的业务键唯一性/无负值/费用口径，后者守 24 条
