...
这是一个由我提出业务目标、持续判断结果并验收，借助 AI 编程助手迭代完成的 **Vibe Coding 毕业设计项目**。项目围绕工业企业能源管理场景，搭建了一条从模拟数据、数据清洗、MySQL 数据仓库、业务分析、异常检测，到 Airflow 调度和可视化报告的完整数据管道。

这里的 Vibe Coding 不是“一句话生成项目”，而是把 AI 当作结对开发者：我负责定义问题、补充工业场景约束、选择统计口径、质疑异常结果并决定取舍；AI 协助编写和重构代码、补测试、整理文档与检查实现。每次修改都通过数据恒等式、查询快照和端到端测试验证，而不是以“代码能运行”作为完成标准。

项目覆盖 8 个车间、6 种能源、2024-01-01 至 2025-12-31 共 731 天的数据，最终形成 29 组分析结果和一份可离线打开的 HTML 报告。它展示的不只是 AI 生成代码的速度，也展示了如何发现 AI 或人工实现中的错误，并用业务规则和自动化测试把结果校准到可信状态。

## 我如何进行 Vibe Coding

| 环节 | 我的职责 | AI 协作内容 | 验证方式 |
|---|---|---|---|
| 需求定义 | 选择工业能耗场景，明确要回答的业务问题 | 将需求拆成数据、仓库、分析、报告和测试任务 | 对照项目目标逐项验收 |
| 业务建模 | 定义车间、能源、生产方式和统计边界 | 生成模型草案、SQL 和实现建议 | 检查物理含义与统计口径 |
| 编码实现 | 决定方案、处理取舍并审阅关键逻辑 | 编写 Python、SQL、前端、Airflow 与测试代码 | 本地运行、查询快照、端到端测试 |
| 问题排查 | 根据异常结果追问原因，判断是真异常还是数据缺陷 | 搜索调用链、比较结果、提出修复方案 | 最小复现、修复前后数据对比 |
| 迭代交付 | 决定哪些结论可以展示以及如何解释限制 | 重构报告、补文档、优化移动端和 CI | 浏览器验收、回归基线、干净环境复现 |

项目中具有代表性的协作闭环，是发现“离群值整行删除”制造了假异常：结果出现不合理的 CUSUM 信号后，我没有直接调大阈值，而是回查数据链路，定位到 34 个车间日缺少能源品种；随后将处理方式改为“异常单元格置空后插补”，并增加回归测试。这类过程比一次生成正确代码更能体现 Vibe Coding 项目中的判断、验证和迭代能力。
...
- 复用同一份内联数据与筛选器引擎的「数据大屏」视图（6 KPI + 7 图，供投屏/看板场景），与报告视图在任意筛选下逐位一致；
