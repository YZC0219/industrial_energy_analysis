...
- [ ] 将现有 MySQL 星型模型迁移为 Hive 的 `ODS → DWD → DWS → ADS` 分层模型，明确各层粒度、主键与数据血缘；
- [ ] 使用 Spark SQL 重写核心清洗和聚合任务，并与 pandas / MySQL 版本进行结果一致性与性能对比；
- [ ] 使用 DataX 构建 MySQL 与 Hive 的离线同步链路，记录全量、增量和失败重跑策略；
- [ ] 扩展现有 Airflow DAG，加入分层依赖、质量门禁、失败告警和补数流程。
