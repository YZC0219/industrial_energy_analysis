...
SELECT d.record_date,round(sum(d.tce),4),round(sum(d.co2_t),3),round(sum(d.cost_yuan),2),
       count(DISTINCT a.workshop_code),
...
LEFT JOIN (SELECT DISTINCT record_date,workshop_code
           FROM energy_dwd.dwd_energy_consumption_detail
           WHERE coalesce(is_deleted,0)=0) a
 ON d.record_date=a.record_date AND d.workshop_code=a.workshop_code
